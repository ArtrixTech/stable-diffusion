...
class SwinIR:
    ...
    def __init__(self, device, model_path, task='real_sr', scale=4, large_model=False, training_patch_size=128, folder_lq='swin_input/', folder_gt=None, folder_output='swin_output/', tile=None):
        self.device = device
        ...
        folder, save_dir, border, window_size = setup(self)

        save_dir = self.folder_output
        ...
        for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
            ...
            imgname, img_lq, img_gt = get_image_pair(
                self, path)  # image to HWC-BGR, float32
            ...
            with torch.no_grad():
                ...
                img_lq = torch.cat([img_lq, torch.flip(img_lq, [3])], 3)[
                    :, :, :, :w_old + w_pad]
                output = test(img_lq, self.model, self, window_size)
            ...
            if img_gt is not None:
                ...
                img_gt = img_gt[:h_old * self.scale,
                                :w_old * self.scale, ...]  # crop gt
                ...
                if self.task in ['jpeg_car', 'color_jpeg_car']:
                    ...
                    if self.task in ['color_jpeg_car']:
        ...
        if img_gt is not None:
            ...
            if self.task in ['jpeg_car', 'color_jpeg_car']:
                ave_psnrb = sum(test_results['psnrb']) / \
                    len(test_results['psnrb'])
...
def main():
    ...
    save_dir = 'results/artrix_output'
...
if __name__ == '__main__':
    # main()
    swin = SwinIR(device=None,model_path='model_zoo/swinir/003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x4_GAN.pth',
